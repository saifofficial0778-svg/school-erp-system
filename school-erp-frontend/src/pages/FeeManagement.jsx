                ...
                return (
                  <tr key={record.student_id || idx} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`font-bold px-2.5 py-0.5 rounded-full uppercase text-[9px] ${record.status === 'paid' ? 'bg-emerald-50 text-emerald-600' : record.status === 'partially_paid' ? 'bg-amber-50 text-amber-600' : 'bg-rose-50 text-rose-600'
                        }`}>{record.status}</span>
                    </td>
                    ...
                    <td className="px-6 py-3 text-center">
                      <button
                        ...
                        className={`text-white text-xs font-bold px-4 py-1.5 rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer ${balance <= 0 && record.status === 'paid' ? 'bg-slate-300 pointer-events-none shadow-none' : 'bg-purple-600 hover:bg-purple-700'
                          }`}
                      >
